...
import com.tp.holdem.model.game.Suit
import io.vavr.collection.List
import spock.lang.Specification
import spock.lang.Unroll

@Unroll
class BestCardCombinationFinderSpec extends Specification {
    def mapCards = { cards ->
        return List.ofAll(cards).map({ code -> Card.coded(code) })
    }

    def "test different (correct) hand combinations"() {
        expect:
        BestCardCombinationFinder.findBestCardsForHand(mapCards(cards), hand).toSet() == mapCards(result).toSet()
    ...
    def "test with incorrect hand combinations"() {
        when:
        BestCardCombinationFinder.findBestCardsForHand(List.ofAll(cards), Hands.ROYAL_FLUSH)
