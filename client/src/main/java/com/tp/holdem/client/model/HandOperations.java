...
import com.tp.holdem.client.compare.CardComparator;
...
public class HandOperations {
	...
	private static boolean isThreeOfAKind(final List<Card> cards) {
		...
		for (int i = 0; i < cards.size() - 2; i++) {
			if (cards.get(i).getHonour() == cards.get(i + 1).getHonour()
				...
				if (i <= 1 && !isThreeOfKind)
					cardsThatMakeDeck.set(i, null);
				else if (isThreeOfKind && i < 5)
					cardsThatMakeDeck.set(i, null);
			}
		}
		cleanDeck();
	...
	public static boolean isAStraight(final List<Card> cards) {
		...
		cleanDeck();
		if (cardsInARow == 6 || cardsInARow == 7)
			trimDeckDown(cardsInARow - 6);
		cleanDeck();
		return isStraight;
	}
	...
	public static boolean isAFlush(final List<Card> cards) {
		...
		if (noOfClubs >= 5) {
			...
			for (int i = 0; i < cardsThatMakeDeck.size(); i++) {
				if (!cardsThatMakeDeck.get(i).getSuit().equals("Club"))
					cardsThatMakeDeck.set(i, null);
			}
			if (noOfClubs > 5)
				trimDeckDown(noOfClubs - 6);
		} else if (noOfHearts >= 5) {
			isFlush = true;
			for (int i = 0; i < cardsThatMakeDeck.size(); i++) {
				if (!cardsThatMakeDeck.get(i).getSuit().equals("Heart"))
					cardsThatMakeDeck.set(i, null);
			}
			if (noOfHearts > 5)
				trimDeckDown(noOfHearts - 6);
		} else if (noOfDiamonds >= 5) {
			isFlush = true;
			for (int i = 0; i < cardsThatMakeDeck.size(); i++) {
				if (!cardsThatMakeDeck.get(i).getSuit().equals("Diamond"))
					cardsThatMakeDeck.set(i, null);
			}
			if (noOfDiamonds > 5)
				trimDeckDown(noOfDiamonds - 6);
		} else if (noOfSpades >= 5) {
			isFlush = true;
			for (int i = 0; i < cardsThatMakeDeck.size(); i++) {
				if (!cardsThatMakeDeck.get(i).getSuit().equals("Spade"))
					cardsThatMakeDeck.set(i, null);
			}
			if (noOfSpades > 5)
				trimDeckDown(noOfSpades - 6);
		}
		cleanDeck();
		return isFlush;
	}
